export const CONTENT_TAG_MAX_LENGTH = 10;
export const CONTENT_TAG_MAX_ITEMS = 5;
export const CONTENT_TAG_ALLOWED_DESCRIPTION = "letters, numbers, hyphen, and underscore";
export const CONTENT_TAG_ALLOWED_INPUT_DESCRIPTION =
...
export function parseContentTagsInput(rawValue) {
...
export function normalizeContentTag(value) {
...
export function validateContentTagValue(value) {
...
export function validateContentTags(values) {
  const items = [];
...
export function resolveContentTagValidationMessage(analysis, { noun = "tags" } = {}) {
  if (!analysis) {
    return "";
  }

  if (analysis.tagCount > CONTENT_TAG_MAX_ITEMS) {
    return `Use up to ${CONTENT_TAG_MAX_ITEMS} ${noun}.`;
  }

  if (analysis.tooLongTags.length > 0) {
    return `Use at most ${CONTENT_TAG_MAX_LENGTH} characters per tag.`;
  }
