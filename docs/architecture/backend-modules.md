...
| `posts` | `POST /posts`, `GET /posts/:id`, `DELETE /posts/:id` | Create post, fetch post details with visible comments, update post moderation metrics, delete posts by owner/moderator/admin with private metric recalculation | `PostRepository`, `CommentService`, `UserService` |
