...
- `feed.html`: unified chronological browsing page for public posts and public collections, with search, a same-page `Posts` vs `Collections` selector, an authenticated followed-tags toggle, manual tag follow form, tag follow/unfollow actions, post create/edit image upload, explicit post-type selection (`regular` vs `questionnaire`) in the shared post modal, questionnaire authoring, questionnaire first-question preview on post cards, visible sequence membership on post cards, and public author chips (avatar, username, reputation tier).
...
- `feed.html` is the shared browsing surface for feed switching and now renders both `Posts` and `Collections` behind the same page-level selector plus an authenticated followed-tags toggle; owner collection creation stays on `collections.html`, which is linked from a single `My collections` header action.
