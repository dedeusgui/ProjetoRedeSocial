...
- `collections.html`: authenticated owner page for collection creation and collection-item management, with a single in-page `New collection` modal trigger, owner cards that keep the same visual cadence as feed/post cards, and follow/unfollow actions on the collection's own tag chips.
- `collection.html`: public collection page with ordered collection items, follow/unfollow actions on the collection's own tag chips for authenticated users, and sequence context for each item, using collection terminology and post-card-like spacing instead of a separate reading-path presentation.
