...
- `core/`
  - session handling, formatter helpers, destructive-action policy, shared UI copy, and API error-to-UI state mapping.
- `components/`
  - reusable UI behavior (navbar state, delegated navigation, flash messages, password visibility toggles, destructive confirmations).
...
- Destructive frontend actions must resolve through the shared destructive-action policy and confirmation helpers; page scripts must not call delete endpoints directly from the initial destructive button click.
...
- Destructive confirmation policy is layered by impact: `comment.delete` uses inline confirmation, `post.delete` and `collection.delete` use strong danger modals, and `account.delete` / `admin.user.delete` stay on strict modal confirmations with extra proof requirements.
