...
- `feed.html` is the shared browsing surface for feed switching and renders both `Posts` and `Collections` behind the same unified header/discovery surface, alongside the authenticated followed-tags toggle and the dedicated followed-tags management modal; owner collection creation stays on `collections.html`, which is linked from a single `My collections` header action.
